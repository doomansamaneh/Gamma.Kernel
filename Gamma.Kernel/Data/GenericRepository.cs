    ...
    where TEntity : BaseEntity, new()
{
    ...
    public async ValueTask<TEntity> InsertAsync(TEntity entity, CancellationToken ct = default)
    {
        ...
        if (cache.IdentityProperty is not null)
        {
            ...
            var id = await uow.Connection.ExecuteScalarAsync(sql, entity, uow.Transaction, commandTimeout: dialect.DefaultCommandTimeout, cancellationToken: ct);
            entity.SetPropertyValue(cache.IdentityProperty.Name, id);
        }
        else
        {
            await uow.Connection.ExecuteAsync(sql, entity, uow.Transaction, commandTimeout: dialect.DefaultCommandTimeout, cancellationToken: ct);
        }
    ...
    public async ValueTask<int> UpdateAsync(TEntity entity, CancellationToken ct = default)
    {
        ...
        var affected = await uow.Connection.ExecuteAsync(sql, entity, uow.Transaction, commandTimeout: dialect.DefaultCommandTimeout, cancellationToken: ct);
    ...
    public async ValueTask<int> DeleteByIdAsync<TKey>(TKey id, CancellationToken ct = default)
    {
        ...
        var sql = $"DELETE FROM {tableName} WHERE {idColumn} = @Id";
        var affected = await uow.Connection.ExecuteAsync(sql, new { Id = id }, uow.Transaction, commandTimeout: dialect.DefaultCommandTimeout, cancellationToken: ct);
